                        ...
                        <TableBody>
                          {(lineMap[e.id] || []).map((l: any) => (
                            <TableRow key={l.id}>
                              ...
                              <TableCell className="text-right">{Number(l.debit) > 0 ? formatCurrency(Number(l.debit)) : "—"}</TableCell>
                              <TableCell className="text-right">{Number(l.credit) > 0 ? formatCurrency(Number(l.credit)) : "—"}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-6 text-sm">
              ...
              <span className={Math.abs(totalDr - totalCr) > 0.01 ? "text-destructive" : "text-emerald-600"}>
                {Math.abs(totalDr - totalCr) > 0.01 ? `Off by ${formatCurrency(Math.abs(totalDr - totalCr))}` : "✓ Balanced"}
              </span>
            </div>
          </div>
