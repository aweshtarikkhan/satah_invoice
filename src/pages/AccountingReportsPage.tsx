                ...
                <TableBody>
                  {data.filter(a => a.debit > 0 || a.credit > 0).map(a => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell className="text-right">{a.debit > 0 ? formatCurrency(a.debit, (org as any)?.currency || "INR") : "—"}</TableCell>
                      <TableCell className="text-right">{a.credit > 0 ? formatCurrency(a.credit, (org as any)?.currency || "INR") : "—"}</TableCell>
                    </TableRow>
                  ))}
                  <TableRow className="font-semibold border-t-2">
                    <TableCell colSpan={3}>Total</TableCell>
                    <TableCell className="text-right">{formatCurrency(totalDebit, (org as any)?.currency || "INR")}</TableCell>
                    <TableCell className="text-right">{formatCurrency(totalCredit, (org as any)?.currency || "INR")}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
              {Math.abs(totalDebit - totalCredit) > 0.01 && (
                <div className="mt-2 text-sm text-destructive">⚠ Out of balance by {formatCurrency(Math.abs(totalDebit - totalCredit), (org as any)?.currency || "INR")}</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
                ...
                <Table>
                  <TableBody>
                    {byType.asset.filter(a => a.balance !== 0).map(a => (
                      <TableRow key={a.id}><TableCell>{a.name}</TableCell><TableCell className="text-right">{formatCurrency(a.balance, (org as any)?.currency || "INR")}</TableCell></TableRow>
                    ))}
                    <TableRow className="font-semibold border-t-2"><TableCell>Total Assets</TableCell><TableCell className="text-right">{formatCurrency(totalAssets, (org as any)?.currency || "INR")}</TableCell></TableRow>
                  </TableBody>
                </Table>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <Table>
                  <TableBody>
                    ...
                    <TableRow className="font-medium"><TableCell>Total Liabilities</TableCell><TableCell className="text-right">{formatCurrency(totalLiab, (org as any)?.currency || "INR")}</TableCell></TableRow>
                    {byType.equity.filter(a => a.balance !== 0).map(a => (
                      <TableRow key={a.id}><TableCell>{a.name}</TableCell><TableCell className="text-right">{formatCurrency(a.balance, (org as any)?.currency || "INR")}</TableCell></TableRow>
                    ))}
                    <TableRow><TableCell>Net Profit (period)</TableCell><TableCell className="text-right">{formatCurrency(netProfit, (org as any)?.currency || "INR")}</TableCell></TableRow>
                    <TableRow className="font-semibold border-t-2"><TableCell>Total Liabilities + Equity</TableCell><TableCell className="text-right">{formatCurrency(totalLiabEq, (org as any)?.currency || "INR")}</TableCell></TableRow>
                  </TableBody>
                </Table>
                {Math.abs(totalAssets - totalLiabEq) > 0.01 && (
                  <div className="mt-2 text-xs text-amber-600">Difference: {formatCurrency(totalAssets - totalLiabEq, (org as any)?.currency || "INR")}</div>
                )}
              </CardContent>
            </Card>
          </div>
              ...
              <Table>
                <TableBody>
                  {data.filter(a => a.code === "1000" || a.code === "1010").map(a => (
                    <TableRow key={a.id}>
                      <TableCell>{a.name}</TableCell>
                      <TableCell className="text-right text-emerald-600">In: {formatCurrency(a.debit, (org as any)?.currency || "INR")}</TableCell>
                      <TableCell className="text-right text-rose-600">Out: {formatCurrency(a.credit, (org as any)?.currency || "INR")}</TableCell>
                      <TableCell className="text-right font-semibold">Net: {formatCurrency(a.balance, (org as any)?.currency || "INR")}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
